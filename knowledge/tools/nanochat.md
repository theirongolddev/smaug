...
stars: 43305
language: "Python"
tags: [LLM-training, harness, Python, GPU-efficient, scaling-laws, open-source, research-tool, leaderboard]
via: "Twitter bookmark from @aakashgupta"
---

The simplest experimental harness for training LLMs with breakthrough cost efficiency. nanochat enables training GPT-2-capable models for ~$72 (3 hours on 8xH100) compared to $43K in 2019. Built for single GPU nodes with emphasis on code simplicity and hackability while maintaining production-ready infrastructure.

**Core Features:**
- One-parameter configuration system (`--depth`) with automatic hyperparameter scaling
- Complete pipeline: tokenization (RustBPE/tiktoken) → pretraining → finetuning → evaluation → inference
- ChatGPT-like web UI for interaction and testing
- FP8 training support for efficiency
- Distributed training with DDP for multi-GPU setups
- Time-to-GPT-2 leaderboard for benchmarking (current record: 2.76 hours for CORE 0.2602)
- Scaling laws computed for model width, heads, learning rate, and training duration

**Use Cases:**
- Rapid prototyping of LLMs on limited budgets
- Research into efficient scaling laws and training dynamics
- Accessible entry point to end-to-end LLM development
- Benchmarking optimization improvements across model sizes
