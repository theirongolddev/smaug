...
source: "https://x.com/i/article/2009497314013138947"
author: "dhasandev"
tags: [claude-code, cli, agentic, automation, tools, structured-output]
via: "Twitter bookmark from @dhasandev"
---

Comprehensive guide to using `claude --print` (`-p`) as a primitive for agentic applications. Covers 5 input methods (CLI arg, stdin pipe, stdin redirect, heredoc, stream-JSON), 3 output formats (text, JSON, stream-JSON with `--verbose`), JSON schema for structured output, tool configuration (whitelist/deny/patterns), permission modes including `bypassPermissions`, session management, custom system prompts, and bidirectional streaming. Key insight: `claude -p` isn't just a CLI wrapper — it's a full-featured agent runtime that happens to be accessible via command line.

## Key Takeaways
- `--output-format stream-json` requires `--verbose` or you get an error; stream input requires stream output (can't mix)
- JSON output includes a `structured_output` field separate from `result` when using `--json-schema`
- Tool control options: `--tools ""` disables all, `--allowedTools "Bash(git:*)"` for pattern-based allow, `--disallowedTools` for deny
- `--permission-mode bypassPermissions` (or `--dangerously-skip-permissions`) enables fully autonomous execution
- Session persistence: `--no-session-persistence` for stateless microservices, `--session-id + --continue` for multi-turn
- `--append-system-prompt` extends Claude's default behavior; `--system-prompt` replaces entirely
- `--fallback-model` provides reliability when primary model is overloaded
- Context caching reduces cost significantly after the first run — avoid dynamic content at the start of prompts
- Background: Claude OAuth tokens from `/login` now only work with Claude Code itself, not third-party wrappers
...
- [Article](https://x.com/i/article/2009497314013138947)
- [Original Tweet](https://x.com/dhasandev/status/2009529865511555506)
