...
source: "https://x.com/i/article/2021152822264201218"
author: "mastra"
tags: [agent-memory, context-engineering, prompt-caching, mastra, long-context, llm-optimization]
via: "Twitter bookmark from @mastra"
---

Mastra's new observational memory system solves context window bloat for agentic AI by compressing conversation history into human-inspired observations. Instead of storing raw messages, the system distills interactions down to key observations (e.g., "User is building a Next.js app due Jan 22 with Supabase auth") using emoji-based priority levels. The approach achieves state-of-the-art results on LongMemEval (94.87% with GPT-4-mini, beating previous SOTA by 3+ points) while remaining fully compatible with prompt caching for cost efficiency.

## Key Takeaways

- Observational memory compresses context the way human memory does—millions of pixels reduce to a few key observations
- Three-part memory architecture: observations (compressed), raw messages (uncompressed), and threshold-based compression when hitting 30k tokens
- Emoji-based prioritization (🔴 important, 🟡 maybe, 🟢 info) acts as log-level replacement—more interpretable than vector stores
- System includes separate "observer" agent (compresses messages) and "reflector" agent (garbage collects stale observations)
- Fully prompt-cacheable: consistent prefix means cache hits on every turn, only full invalidation during reflection
- Open-source implementation available; designed for compatibility with Anthropic, OpenAI, and other providers
- Solves the real problem: browser agents, coding agents, research agents all generate massive context quickly
...
- Formatted text, not structured objects. Down with knowledge graphs. Roon was right. [Text is the universal interface](https://scale.com/blog/text-universal-interface). It's easier to use, optimized for LLMs, and far easier to debug.
...
Mastra's new memory system, Observational Memory, achieves 94.87% on [LongMemEval](https://arxiv.org/abs/2410.10813) with gpt-5-mini — over 3 points higher than any previously recorded score. With gpt-4o (the standard benchmark model), it scores 84.23%, beating the gpt-4o oracle (a configuration given only the conversations containing the answer) by 2 points, and the previous gpt-4o SOTA from Supermemory by 2.6 points.

These scores were achieved with a completely stable context window. The context is predictable, reproducible, and fully prompt-cacheable. See the [full research breakdown](https://mastra.ai/research/observational-memory).
...
Full per-category breakdowns and methodology are in the [research page](https://mastra.ai/research/observational-memory).
...
- [Article](https://x.com/i/article/2021152822264201218)
- [Original Tweet](https://x.com/mastra/status/2021280193273336131)
