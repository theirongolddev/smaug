...
source: "https://x.com/i/article/2010809702980935681"
author: "eyad_khrais"
tags: [claude-code, skills, subagents, mcp, ai-agents, tutorials, advanced-usage]
via: "Twitter bookmark from @eyad_khrais"
---

## Summary

This advanced Claude Code tutorial builds on fundamentals to explain three powerful capabilities: skills, subagents, and MCP connectors. Based on 7 years of SWE experience at Amazon, Disney, and Capital One, the article explains how these systems fit together in practice and which configurations matter for production work. The key insight is that context degradation at 45% of the context window can be solved with subagents, allowing you to maintain clean conversations while delegating complex tasks.

## Key Takeaways

- **Context Window Advantage**: Claude Code delivers a consistent 200K token context window, while other tools like Cursor often truncate internally to 70-120K, making it critical for large interconnected codebases
- **Skills Architecture**: Markdown files in ~/.claude/skills/ teach Claude project-specific workflows with YAML frontmatter (name, description). Claude automatically applies relevant skills without token bloat via progressive disclosure
- **Subagents Pattern**: Separate Claude instances with isolated context windows prevent context pollution on long sessions. Built-in options include Explore (read-only), Plan (research), and General-purpose agents
- **MCP Connectors**: Standardized protocol for integrating external services (GitHub, Slack, databases, issue trackers) without context switching, enabling seamless multi-tool workflows
- **Compound Effect**: Skills encode patterns + subagents handle subtasks + MCP connects services = a system that improves with use and keeps attention on implementation work
...
Part one went mega viral, but if you missed it, read it first here: https://x.com/eyad_khrais/status/2010076957938188661?s=20
...
- [Article](https://x.com/i/article/2010809702980935681)
- [Original Tweet](https://x.com/eyad_khrais/status/2010810802023141688)
