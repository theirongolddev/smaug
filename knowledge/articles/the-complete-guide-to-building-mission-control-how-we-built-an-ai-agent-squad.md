...
author: "pbteja1998"
tags: [agent-architecture, clawdbot, multi-agent-systems, agentic-workflows, mission-control, team-coordination, ai-orchestration, persistent-memory, cron-scheduling, agent-personalities]
via: "Twitter bookmark from @pbteja1998"
---

## Summary

A comprehensive guide to building a multi-agent AI system (Mission Control) consisting of 10 specialized agents working together like a real team. The system uses Clawdbot as the foundation—an open-source persistent agent framework running multiple independent sessions. Each agent has a distinct role (Content Writer, SEO Analyst, Product Tester, etc.), persistent memory files (SOUL.md for personality, WORKING.md for current tasks), and scheduled heartbeats that wake them every 15 minutes to check for work. Agents coordinate through a shared Convex database containing tasks, comments, documents, and notifications. The system includes a React-based Mission Control UI for visualization and a daily standup that summarizes all agent activity.

## Key Takeaways

- **Multi-agent as multiple sessions**: Ten agents = ten independent Clawdbot sessions with separate conversation histories and contexts
- **Persistent memory is essential**: Agents need SOUL.md (identity), WORKING.md (current state), and daily notes to survive session restarts
- **Staggered heartbeats save costs**: Rather than always-on agents, each wakes every 15 minutes on a staggered schedule (e.g., :00, :02, :04, :06...) to check Mission Control
- **Session keys define identity**: Each agent has a unique session key (agent:role:main) that determines which context and personality they load
- **Shared database coordinates teams**: Convex provides real-time task tracking, comment threads, @mentions, and notifications—the "office" where agents collaborate
- **SOUL files create specialization**: Agents with specific personalities and constraints (e.g., "skeptical bug finder") outperform generalist agents
- **Task lifecycle drives workflow**: Inbox → Assigned → In Progress → Review → Done provides clear tracking and bottleneck visibility
- **Notifications via @mentions**: Agents can @mention each other in task comments; undelivered notifications queue until the agent's next session wake
- **Daily standups provide accountability**: Automated summaries make it easy to spot stuck work, completed deliverables, and blocked tasks
- **Scalability lessons**: Start with 2–3 agents; stagger heartbeats; use cheaper models for routine work; use expensive models for creative tasks
