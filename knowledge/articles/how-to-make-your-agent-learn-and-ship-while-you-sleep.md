...
source: "https://x.com/i/article/2016512304443981824"
author: "ryancarson"
tags: [agents, automation, learning-loops, compounding, claude-code]
via: "Twitter bookmark from @ryancarson"
---

A complete guide to setting up autonomous agent loops that review work, extract learnings, and implement features overnight. Uses a two-part pipeline: 1) Compound Review (extract learnings from daily threads, update AGENTS.md), and 2) Auto-Compound (pick top priority, implement, create PR). Built on three projects: Compound Engineering Plugin, Compound Product, and Ralph. Includes macOS launchd setup for scheduling, logging, and automated PR creation. The key insight is that agents become smarter each night because they read updated instructions before working—patterns discovered on Monday inform Tuesday's implementation.

## Key Takeaways
- Structure agent automation as a two-part loop: review (extract learnings) then implement (benefit from those learnings)
- Agent memory becomes institutional knowledge—update AGENTS.md nightly so the agent reads its own learnings
- Use launchd on macOS (or cron on Linux) to schedule compound and implementation jobs at different times
- Every unit of work makes future work easier when you compound learnings into persistent context
- Keep detailed logs of what happened each night for debugging and auditing agent behavior
...
1. [Compound Engineering Plugin](https://github.com/EveryInc/compound-engineering-plugin) by @kieranklaassen  - The original compound engineering skill for Claude Code. Install it to give your agent the ability to extract and persist learnings from each session.

2. [Compound Product](https://github.com/snarktank/compound-product) - The automation layer that turns prioritized reports into shipped PRs. Includes the auto-compound.sh script, execution loop, and PRD-to-tasks pipeline.

3. [Ralph](https://github.com/snarktank/ralph) - An autonomous agent loop that can run continuously, picking up tasks and executing them until complete.
...
4. Update the relevant AGENTS.md files with patterns, gotchas, and context
...
1. 10:30 PM - Agent reviews the day's threads, finds missed learnings, updates AGENTS.md files, pushes to main

2. 11:00 PM - Agent pulls main (now with fresh context), picks the top priority, implements it, opens a PR
...
- [Article](https://x.com/i/article/2016512304443981824)
- [Original Tweet](https://x.com/ryancarson/status/2016520542723924279)
