...
article_id: "2015775451810246656"
author: "@manthanguptaa"
tags: [ai, memory-systems, clawdbot, personal-assistant, sqlite, embeddings, local-first]
via: "Twitter bookmark from @manthanguptaa"
---

## Summary

Deep dive into Clawdbot's persistent memory system - how an open-source personal AI assistant maintains 24/7 context retention using plain Markdown files, SQLite vector search, and a two-layer memory architecture. Key innovations: memory is transparent and editable (just Markdown), hybrid search (semantic + BM25), compaction with pre-flush memory preservation, and multi-agent memory isolation. The system demonstrates how to build production-grade AI memory without cloud dependencies.

## Key Takeaways

- **Two-layer memory**: Daily logs (memory/YYYY-MM-DD.md) + curated long-term (MEMORY.md)
- **Transparent storage**: Memory is plain Markdown in the agent workspace, version-controllable
- **Hybrid search**: 70% semantic (vector) + 30% keyword (BM25) with sqlite-vec and FTS5
- **Smart compaction**: Pre-compaction memory flush ensures important info is saved before lossy summarization
- **Local-first**: Everything runs on your machine with full ownership of context and skills
- **Multi-agent isolation**: Each agent gets its own workspace and SQLite index

## Technical Architecture

**Memory storage:**
- Layer 1: `memory/YYYY-MM-DD.md` (append-only daily notes)
- Layer 2: `MEMORY.md` (curated persistent knowledge)
- Indexes: `~/.clawdbot/memory/<agentId>.sqlite` with chunks, embeddings, and FTS5

**Chunking strategy:**
- ~400 token chunks with 80 token overlap
- Balances semantic coherence vs granularity
- Overlap captures facts spanning boundaries

**Search mechanism:**
- `memory_search`: Semantic search across all files
- `memory_get`: Read specific lines after finding them
- finalScore = (0.7 * vectorScore) + (0.3 * textScore)
- Filters results below minScore threshold (default 0.35)

**Compaction with memory preservation:**
- Triggers when approaching context limit
- Pre-compaction flush: Agent silently writes important info to disk
- Summarizes older conversation while keeping recent intact
- Summary persists to JSONL transcript for future sessions
...
- [X Article](https://x.com/i/article/2015775451810246656)
