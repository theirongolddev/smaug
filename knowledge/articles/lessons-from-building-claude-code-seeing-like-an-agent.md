...
author: "trq212"
tags: [claude-code, agent-design, tools, elicitation, progressive-disclosure, tool-design]
via: "Twitter bookmark from @trq212"
---

Internal lessons from the Claude Code team on designing agent action spaces — specifically, how to design tools that fit the model's actual capabilities rather than human intuitions. Three case studies: (1) AskUserQuestion tool evolution through failed attempts (adding to ExitPlanTool confused scope; markdown output format was unreliable) to a dedicated blocking tool; (2) TodoWrite → Task Tool upgrade as Opus 4.5 no longer needed reminders and needed subagent coordination instead; (3) RAG → Grep → progressive disclosure for context building as models got better at self-directing search. Core principle: "see like an agent" — read outputs, experiment, adapt tools to what the model can actually use effectively.

## Key Takeaways
- Design tools to fit the model's abilities, not human intuitions — what works for one capability level may actively constrain a more capable model
- AskUserQuestion lesson: structured tool beats format-parsing; Claude reliably calls tools better than it reliably follows output format instructions
- TodoWrite → Task Tool: as models improved, reminder injections made Claude think it couldn't deviate from the list; Tasks enable subagent coordination
- Progressive disclosure: Claude can now do nested multi-layer file search to build its own context — RAG setup/fragility is no longer worth the tradeoff
- The Claude Code Guide subagent (called when you ask Claude about itself) is progressive disclosure applied to meta-knowledge: avoids context rot from rarely-needed docs
- Bar to add a new tool is high — more options = more cognitive load on the model; prefer progressive disclosure over new tools
- The right tool set changes as model capabilities improve — constant revisiting of assumptions is required
