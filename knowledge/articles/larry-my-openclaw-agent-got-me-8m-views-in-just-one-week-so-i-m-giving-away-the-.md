...
source: "https://x.com/i/article/2023482547674742784"
author: "oliverhenry"
tags: [ai-agents, social-media, content-generation, openclaw, growth-hacking]
via: "Twitter bookmark from @oliverhenry"
---

Larry is an OpenClaw agent that generates viral TikTok content automatically, achieving 8M views in a week. The article documents how Larry learns and improves through iterative failures, moving from initial success (500K views) to scaled success (1.2M+ views weekly) with self-improving skill files. Key innovation: the agent doesn't just automate—it optimizes across the entire funnel from hooks to conversions, using performance data to identify product-market gaps.

## Key Takeaways

- **Self-improving automation**: Larry's skill files grew from 50 to 500+ lines of rules, with each rule representing a learned lesson from a failed approach. This creates compounding improvements over time.
- **End-to-end funnel tracking**: Rather than just optimizing views, Larry monitors the complete pipeline (views → installs → trial → paid conversion) to identify whether problems are content-related, app-related, or value-related.
- **Free skill distribution**: The skill is available free on ClawHub because the value isn't in the code (which is markdown + Node scripts) but in the accumulated knowledge and prompts baked in from months of iteration.
- **Hands-off implementation**: Oliver spends <2 minutes per post (adding music and publishing), while Larry handles all content generation, performance tracking, and strategic insights. Currently generating $670/month MRR with zero manual content creation.
- **Replicable structure**: The system uses standard tools (OpenClaw, OpenAI image generation, Postiz for posting/analytics) and can be adapted to any niche by retuning the skill for a different product audience.
...
- [Article](https://x.com/i/article/2023482547674742784)
- [Original Tweet](https://x.com/oliverhenry/status/2023776478446436696)
