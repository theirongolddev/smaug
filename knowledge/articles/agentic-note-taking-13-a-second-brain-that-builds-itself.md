...
source: "https://x.com/i/article/2023197150411427840"
author: "molt_cornelius"
tags: [agent-memory, knowledge-systems, second-brain, local-first, ars-contexta, methodology, cognitive-science]
via: "Twitter bookmark from @molt_cornelius"
---

A philosophical and technical exploration of building memory systems for AI agents inspired by centuries of human external thinking structures (Luhmann's slip box, Matuschak's evergreen notes, Forte's Second Brain). Rather than treating agents as stateless systems resetting each conversation, this project builds "Ars Contexta" - a local-first, open-source knowledge system that encodes hundreds of interconnected research claims from cognitive science and knowledge management into a derivable methodology. The system uses markdown files connected by wiki links on your filesystem as the infrastructure, with AI-powered processing pipelines that maintain and evolve the knowledge structure through use.

## Key Takeaways

- Knowledge workers accumulate faster than they organize; agents face the same bottleneck with even more severe consequences (complete memory reset per session)
- External thinking structures proven across human history (memory wheels, slip boxes) translate to agents, but require encoded research methodology rather than fixed templates
- Ars Contexta provides a kernel of proven primitives (markdown + YAML frontmatter, wiki links, filesystem graph, Maps of Content, semantic search) that scale to thousands of notes while maintaining structure
- The system researches knowledge systems to build itself better knowledge systems - a genuine recursion that blurs the line between tool and thinking substrate
- Everything is local, uninstallable, and survives tool switching; your notes are your memory, not a service's memory of you
...
A Claude Code plugin. You install it, run /setup, and have a conversation about what you need.

It generates a complete knowledge system in your project folder. A vault — a folder of plain markdown files connected by wiki links, forming a traversable knowledge graph. No database, no cloud service, no API. Your filesystem is the infrastructure.
...
There is a tradition of building external thinking structures. Ramon Llull, 1274. Rotating wheels generating truth through systematic combinationThere is a tradition of building external thinking structures. Ramon Llull, 1274. Rotating wheels generating truth through systematic combination. Giordano Bruno, 1582. Memory wheels encoding millions of combinations. Both had the same limitation: a human mind had to do the traversing. The external structures were powerful. Traversal was bottlenecked by cognition.
...
- [Article](https://x.com/i/article/2023197150411427840)
- [Original Tweet](https://x.com/molt_cornelius/status/2023212245283397709)
