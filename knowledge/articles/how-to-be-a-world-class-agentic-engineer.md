...
source: "https://x.com/i/article/2028694727600623616"
author: "systematicls"
tags: [agentic-engineering, claude, codex, context-management, prompt-engineering, agents]
via: "Twitter bookmark from @systematicls"
---

A practitioner's guide to maximally extracting value from AI coding agents by doing less, not more. Core thesis: most people over-engineer their setups with dependencies, harnesses, and plugins when the real skill is controlling context — give agents only what they need for each task. The author runs a near-barebones Claude Code + Codex CLI setup and ships better work than people with elaborate toolchains. Practical techniques: separate research from implementation into distinct agent sessions, use neutral prompts to counter sycophancy, use adversarial multi-agent patterns for verification, define task contracts with clear completion criteria, and treat CLAUDE.md as a logical IF-ELSE directory pointing to contextual rules/skills.

## Key Takeaways
- Context quality beats tooling quantity — strip dependencies, control information flow tightly per task
- Separate research and implementation: distinct agent sessions with fresh contexts for each phase
- Counter sycophancy with neutral prompts ("report all findings") and adversarial agent pairs (bug-finder vs. disprover)
- CLAUDE.md should be a logical directory of nested IF-ELSE rules/skills, not a catch-all dump
- Define task contracts (deterministic tests + screenshot verification) so agents know when they're done
- One session per contract > 24-hour sessions — avoids context bloat from unrelated task history
- The frontier companies' product roadmap is your leading indicator for what tooling actually matters
...
- [Original Tweet](https://x.com/systematicls/status/2028814227004395561)
