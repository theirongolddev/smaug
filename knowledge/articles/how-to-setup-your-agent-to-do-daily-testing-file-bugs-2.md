...
author: "ryancarson"
tags: [testing, e2e, agents, automation, ci-cd, oauth, quality-assurance]
via: "Twitter bookmark from @ryancarson"
---

Agent-driven E2E testing approach that runs daily to catch regressions missed by traditional tests. Unlike brittle CSS selectors, uses accessibility tree snapshots and adapts to semantic changes. Covers full signup/onboarding flows with fresh test users each run, unattended Google OAuth via pre-signed Chrome profile, and auto-filing bugs in your tracker on failure—all implementable in 1 hour.

## Key Takeaways
- Agent-driven E2E adapts to semantic UI changes vs brittle selector dependencies
- Unattended Google OAuth using pre-signed Chrome debug profile with stored credentials
- Fresh test user for each run with cleanup across DB, auth, and payment providers
- Automatic bug filing when tests fail, with deduplication to prevent noise
- Works with any issue tracker (GitHub, Linear, Jira) via API
- Runs daily via launchd, catches regressions developers would miss for days
