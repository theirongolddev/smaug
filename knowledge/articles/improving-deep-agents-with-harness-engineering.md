...
source: "https://x.com/i/article/2022906014928904192"
author: "Vtrivedy10"
tags: [ai-agents, harness-engineering, coding-agents, self-verification, benchmarks, agent-design]
via: "Twitter bookmark from @Vtrivedy10"
---

Harness engineering demonstrates that LLM model improvements aren't the only lever for agent performance—the system architecture around the model matters equally or more. By improving only the harness (system prompts, tools, and middleware) while keeping the base model fixed, a coding agent improved from 52.8% to 66.5% on Terminal Bench 2.0, moving from outside Top 30 to Top 5. The key technique is using trace analysis to identify failure patterns at scale, then iteratively refining the harness based on systematic error analysis.

## Key Takeaways

- **Harness engineering is model-agnostic optimization**: The system prompt, tool selection, and middleware (hooks around model and tool calls) can yield dramatic performance improvements without changing the underlying LLM, suggesting the model is not the bottleneck.
- **Trace-driven improvement loop**: Using LangSmith traces to systematically collect failures, spawning parallel analysis agents to categorize errors, then synthesizing findings for targeted harness changes creates a repeatable improvement methodology.
- **Three critical knobs**: System prompts, tools, and middleware are the highest-leverage intervention points. Deliberately constraining the optimization space to these three areas makes improvements more systematic and reproducible than tuning everything at once.
- **Self-verification matters**: The paper highlights self-verification and tracing as key techniques, suggesting agents perform better when given tools to check their own work and when their reasoning is observable and analyzable.
- **Benchmarks as feedback loops**: Terminal Bench 2.0, Harbor orchestration, and LangSmith observability create a complete system for rapid experimentation, allowing researchers to test harness changes against 89 coding tasks across multiple domains.
...
- [Article](https://x.com/i/article/2022906014928904192)
- [Original Tweet](https://x.com/Vtrivedy10/status/2023805578561060992)
