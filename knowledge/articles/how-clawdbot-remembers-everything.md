...
source: "https://x.com/i/article/2015775451810246656"
author: "manthanguptaa"
tags: ["ai-memory", "context-management", "persistent-memory", "local-ai", "agents", "clawdbot", "semantic-search", "architecture"]
via: "Twitter bookmark from @manthanguptaa"
---

## Summary

This article breaks down Clawdbot's memory architecture, showing how a local AI assistant maintains 24/7 context retention across unlimited sessions. Unlike cloud-based memory (ChatGPT, Claude) controlled by platforms, Clawdbot keeps everything local as plain Markdown files, giving users full ownership. The system uses a two-layer memory structure (daily logs + curated long-term memory), hybrid search (vector + BM25 keyword matching), and intelligent context management (compaction, memory flush, pruning) to balance cognitive continuity with token limits and API costs. It demonstrates that sophisticated, stateful AI agents can run locally without surrendering cognitive data to platforms.

## Key Takeaways

- **Two-layer memory**: Daily logs (memory/YYYY-MM-DD.md) for ongoing context + MEMORY.md for curated long-term knowledge, both searchable
- **Plain Markdown is the source of truth**: All memory is editable, transparent, version-controlled—no opaque databases or proprietary formats
- **Hybrid search wins**: Vector similarity (70% weight) catches semantic meaning while BM25 keyword matching (30% weight) catches exact terms, dates, names
- **Memory is indexed in SQLite**: Chunks (~400 tokens with 80-token overlap) are embedded and stored with both vector and FTS5 indexes for fast retrieval
- **Pre-compaction memory flush**: Before context becomes stale through compaction, the agent saves important decisions/facts to disk, preventing knowledge loss
- **Automatic compaction**: When approaching context limits, older turns are summarized while recent messages stay intact; summaries persist to disk
- **Cache-TTL pruning**: Detects when Anthropic's prompt cache expires and trims old tool results before re-caching, reducing costs on idle sessions
- **Soft sandboxing with isolation**: Each agent has its own workspace + memory index; agents don't cross-pollinate memories by default
- **Session memory hook**: When `/new` starts a fresh session, important context is automatically saved with LLM-generated slug for later retrieval
...
- [Clawdbot Documentation](https://docs.clawd.bot/) - Official docs covering setup, configuration, and all features

- [GitHub Repository](https://github.com/clawdbot/clawdbot) - Source code, issues, and community contributions

---

If you found this interesting, I would love to hear your thoughts. Share it on [Twitter](https://twitter.com/manthanguptaa), [LinkedIn](https://www.linkedin.com/in/manthanguptaa/), or reach out at guptaamanthan01[at]gmail[dot]com.

You can find more blogs from me on https://manthanguptaa.in/

## Links

- [Article](https://x.com/i/article/2015775451810246656)
- [Original Tweet](https://x.com/manthanguptaa/status/2015780646770323543)
