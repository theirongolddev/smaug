...
source: "https://nesbitt.io/2026/02/26/git-in-postgres.html"
author: "Andrew Nesbitt"
tags: [postgres, git, forgejo, gitgres, self-hosting, federation, database, devtools]
via: "Twitter bookmark from @samhogan"
---

Andrew Nesbitt's [gitgres](https://github.com/nicowillis/gitgres) proves that git's object model maps cleanly to two Postgres tables — `objects` and `refs` — and that libgit2's pluggable backend interfaces let a standard git client push and clone from a Postgres-backed remote without modification. The article walks through a proposed Forgejo fork where the 15,000-line `modules/git` shell-out layer is replaced with database queries, collapsing deployment to a single `pg_dump`-able Postgres instance. Postgres primitives cover much of what forges build custom infrastructure for: `NOTIFY` triggers for push events, row-level security for multi-tenancy, logical replication for partial mirroring, and `pg_trgm` for cross-repo search.

## Key Takeaways
- Git's full data model fits in two SQL tables: `objects` (sha1-addressed blobs) and `refs` (compare-and-swap pointers)
- libgit2 backend interfaces make the storage layer swappable — git clients see no difference
- A Forgejo fork replacing `modules/git` with Postgres queries would eliminate the filesystem/database split that complicates backups, replication, and read scaling
- Postgres NOTIFY on the refs table replaces custom webhook polling; RLS handles multi-tenant isolation; logical replication enables selective repo mirroring
- Storage overhead vs packfile delta-compression is real but acceptable for small instances; delta-repacking or S3 offload are the natural next steps
- The vision: `docker compose up` forges backed by a single Postgres instance — critical for federation and decentralized hosting
...
- [Article](https://nesbitt.io/2026/02/26/git-in-postgres.html)
- [Original Tweet](https://x.com/samhogan/status/2030490812547793404)
