...
author: "koylanai"
tags: [context-engineering, ai-agents, pkm, file-system, architecture, agent-memory, skills, personal-os]
via: "Twitter bookmark from @koylanai"
---

Koylan details building "Personal Brain OS" — a Git-based personal operating system for AI agents built from 80+ markdown, YAML, and JSONL files. The core insight is context engineering beats prompt engineering: instead of writing better prompts, design information architecture so the model always has exactly what it needs. The system uses 11 isolated modules with progressive disclosure (3 levels: routing → module instructions → data), file formats chosen for agent compatibility, and a skill system combining auto-loading reference skills with manual slash-command task skills.

## Key Takeaways
- **Context engineering > prompt engineering**: design information architecture, not individual prompts — the shift is from optimizing interactions to designing a filing system
- **Progressive disclosure**: 3 levels (lightweight routing file → module instructions → actual data) mean the model loads only what the current task requires, never more
- **Format-function mapping**: JSONL for append-only safety (agents can add, never overwrite), YAML for hierarchical config with comments, Markdown for LLM-native narrative
- **Module isolation**: 11 separate modules prevent context bloat and instruction conflicts; module boundaries are loading decisions — get them wrong and performance degrades
- **Append-only is non-negotiable**: agents can add data but never destroy it; JSONL makes this structural and prevents a whole category of data loss bugs
- **Voice as structured data**: 5 attributes on 1-10 scales + 50+ banned words beats vague adjectives like "professional but approachable" — banned word lists define what you're NOT, which is more actionable
- **Episodic memory stores judgment, not just facts**: `decisions.jsonl`, `experiences.jsonl`, `failures.jsonl` encode how you actually think about tradeoffs, not just what happened
- **Skill system duality**: auto-loading reference skills (voice guide, anti-patterns) activate silently on every writing task; slash-command task skills give precision control over complex workflows
...
[Embedded Tweet: https://x.com/i/status/2005827257458131321]
...
[Embedded Tweet: https://x.com/i/status/1999192104850133146]
...
[Embedded Tweet: https://x.com/i/status/1998530190847390025]
...
[Embedded Tweet: https://x.com/i/status/1997444237890081104]
...
[Embedded Tweet: https://x.com/i/status/2023405681080938932]
...
[Embedded Tweet: https://x.com/i/status/1996905189656211931]
...
[Embedded Tweet: https://x.com/i/status/1996757974610559171]
...
[Embedded Tweet: https://x.com/i/status/2005082048973905938]
...
[Embedded Tweet: https://x.com/i/status/2016684758588154239]
...
[Embedded Tweet: https://x.com/i/status/2008824728824451098]
...
[Embedded Tweet: https://x.com/i/status/1975090268316827983]
...
[Embedded Tweet: https://x.com/i/status/2018865011356053927]
...
[Embedded Tweet: https://x.com/i/status/1987254288393916571]
...
[Embedded Tweet: https://x.com/i/status/2002797649842331919]

## Links

- [Article](https://x.com/i/article/2025249985722224640)
- [Original Tweet](https://x.com/koylanai/status/2025286163641118915)
