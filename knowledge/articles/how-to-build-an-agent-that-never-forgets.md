...
author: "rohit4verse"
tags: ["agents", "memory", "vector-db", "knowledge-graph", "architecture", "llm", "state-machines"]
via: "Twitter bookmark from @rohit4verse"
---

Deep technical guide to building agent memory systems that persist and learn across weeks and thousands of sessions. Identifies the core problem: embeddings measure similarity, not truth—standard RAG approaches hallucinate when data conflicts. Provides two production architectures: file-based memory (self-organizing hierarchical system for assistants) and context-graph memory (precise knowledge webs with conflict resolution). Covers checkpointing for short-term state, tiered retrieval for efficiency, and essential maintenance (nightly consolidation, weekly summarization, monthly re-indexing).

## Key Takeaways

- **Memory as infrastructure, not feature**: Requires systematic architecture comparable to OS-level concerns (process management, memory allocation, I/O)
- **Two proven architectures**: File-based (hierarchical resources/items/categories) for narrative coherence vs. knowledge-graph (subject-predicate-object with conflict resolution) for precision
- **Checkpointing**: Snapshots of complete agent state enable determinism, recoverability, and debuggability for short-term continuity
- **Tiered retrieval**: Summaries first → sufficiency check → drill to items → fallback to resources avoids token waste and speeds inference
- **Five critical mistakes**: Storing raw conversations, blind embedding usage, no decay, no write rules, treating memory as chat history
- **Essential maintenance**: Nightly consolidation (merge duplicates, promote hot memories), weekly summarization (compress old, prune stale), monthly re-indexing (rebuild embeddings, archive dead nodes)
- **Time decay function**: Recent memories often outrank perfect matches from months ago—use temporal weighting in ranking
...
- [Article](https://x.com/i/article/2008980750738313221)
